...
namespace PluginFileReader.Helper
{
    public class Settings
    {
        ...
        {
            var noCommentsQuery = RemoveComments(query, true).Replace('\n', ' ');
            
            var fromSplits = noCommentsQuery.ToLower().Split("from");
            var joinSplits = noCommentsQuery.ToLower().Split("join");
            ...
            foreach (var selectSplit in fromSplits.Skip(1))
            {
                ...
                rootPaths.AddRange(RootPaths.FindAll(r =>
                    tableName.Contains(new DirectoryInfo(r.RootPathName()).Name.ToLower())));
            }
        ...
        private string RemoveComments(string input, bool preservePositions, bool removeLiterals = false)
        {
            ...
            var lineCommentsOnLastLine =
                @"--(.*?)$"; // because it's possible that there's no \r\n after the last line comment
            ...
            var quotedIdentifiers =
                @"(\""((\""\"")|[^""])*\"")"; // "object", "object[]", etc - when QUOTED_IDENTIFIER is set to ON, they are identifiers, else they are literals
            ...
            var noComments = Regex.Replace(input,
                ...
                me =>
                {
                    if (me.Value.StartsWith("/*") && preservePositions)
                        return
                            everythingExceptNewLines.Replace(me.Value,
                        ...
                        return me.Value;
                    else if (removeLiterals && preservePositions
